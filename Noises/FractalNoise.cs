...
namespace UniNoise.Noises
{
    public sealed class FractalNoise : IDisposable
    {
        ...
        public FractalNoise(int seed, float scale, int width, int height, int octaves, float lacunarity, float persistence)
        {
            _seed = seed;
            _scale = scale;
            _width = width;
        ...
        {
            if (!_arraysInitialized || _noiseValuesNative.Length != _width * _height)
            {
