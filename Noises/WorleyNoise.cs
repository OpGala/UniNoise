...
namespace UniNoise.Noises
{
    public sealed class WorleyNoise : System.IDisposable
    {
        ...
        public WorleyNoise(int seed, int width, int height, int numCells, float scale, float jitter, DistanceFunction distanceFunction, int numberOfFeatures)
        {
            _seed = seed;
            _width = width;
            _height = height;
        ...
        internal float GetDistance(float3 point, float3 cellPoint)
        {
            switch (_distanceFunction)
            {
                case DistanceFunction.Manhattan:
                    return math.abs(point.x - cellPoint.x) + math.abs(point.y - cellPoint.y);
                case DistanceFunction.Chebyshev:
                    return math.max(math.abs(point.x - cellPoint.x), math.abs(point.y - cellPoint.y));
        ...
        private struct WorleyNoiseJob : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                for (int i = 0; i < NumCells; i++)
                {
                    float3 jitteredPoint = CellPoints[i] + new float3(Jitter * Random.CreateFromIndex((uint)(index + i)).NextFloat2Direction(), 0);
                ...
                NoiseValues[index] = distances[NumberOfFeatures - 1] / math.sqrt(Width * Width + Height * Height);
                distances.Dispose();
            }

            private static float GetDistance(float3 point, float3 cellPoint, DistanceFunction distanceFunction)
            {
