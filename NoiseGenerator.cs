...
{
      [ExecuteInEditMode]
      public sealed class NoiseGenerator : MonoBehaviour, IDisposable
      {
            ...
            public void Dispose()
            {
                  this._noiseValuesNative.Dispose();
                  this._colorsNative.Dispose();
            }

            private void InitializeArrays()
            {
                  if (_arraysInitialized && _noiseValuesNative.Length == combinedNoiseConfig.noiseConfigurations[0].width * combinedNoiseConfig.noiseConfigurations[0].height)
                  {
                        return;
                  }
            ...
            {
                  if (combinedNoiseConfig.noiseConfigurations.Count == 0 || combinedNoiseConfig.noiseConfigurations[0].width <= 0 || combinedNoiseConfig.noiseConfigurations[0].height <= 0)
                  {
                        return;
                  }
                  ...
                  {
                        float[] noiseValues = Noise.GetNoise(config.noiseType, config.seed, config.width, config.height, config.scale, config.octaves, config.lacunarity, config.gain, config.gain,
                              config.amplitude, config.frequency, config.bias, config.numCells, config.jitter, config.distanceFunction, config.numberOfFeatures, config.orientation, config.aspectRatio,
