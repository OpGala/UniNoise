...
namespace UniNoise
{
          ...
          public static class Noise
          {
                    ...
                    {
                              return GetNoise(config.noiseType, config.seed, config.width, config.height, config.scale,
                                                  config.octaves, config.lacunarity, config.gain, config.persistence,
                                                  config.amplitude,
                                                  config.frequency, config.bias, config.numCells, config.jitter,
                                                  config.distanceFunction,
                                                  config.numberOfFeatures, config.orientation, config.aspectRatio,
                                                  config.phase,
                    ...
                    /// <returns>An array of floats representing the generated noise map.</returns>
                    public static float[] GetNoise(NoiseType type = NoiseType.Perlin, int seed = 1, int width = 256,
                                        int height = 256, float scale = 1.0f,
                                        int octaves = 1, float lacunarity = 1.0f, float gain = 0.5f,
                                        float persistence = 1.0f, float amplitude = 1.0f, float frequency = 1.0f,
                                        float bias = 0.0f,
                                        int numCells = 64, float jitter = 1.0f,
                                        DistanceFunction distanceFunction = DistanceFunction.Euclidean,
                                        int numberOfFeatures = 1, float orientation = 0.0f, float aspectRatio = 1.0f,
                                        float phase = 0.0f, int kernelSize = 3, float2 offset = default)
                    {
                              switch (type)
                              {
                                        ...
                                        case NoiseType.PerlinFractal:
                                                  var perlinFractal = new PerlinFractalNoise(seed, scale, width, height,
                                                                      lacunarity, gain, octaves);
                                                  return perlinFractal.Generate();

                                        case NoiseType.Worley:
                                                  var worley = new WorleyNoise(seed, width, height, numCells, scale,
                                                                      jitter, distanceFunction, numberOfFeatures);
                                        ...
                                        case NoiseType.SimplexFractal:
                                                  var simplexFractal = new SimplexFractalNoise(seed, scale, width,
                                                                      height, octaves, lacunarity, persistence,
                                                                      amplitude, frequency);
                                        ...
                                        case NoiseType.Value:
                                                  var value = new ValueNoise(seed, scale, width, height, octaves,
                                                                      lacunarity, persistence);
                                                  return value.Generate();

                                        case NoiseType.Wavelet:
                                                  var wavelet = new WaveletNoise(seed, scale, width, height, octaves,
                                                                      lacunarity, persistence);
                                                  return wavelet.Generate();

                                        case NoiseType.Fractal:
                                                  var fractal = new FractalNoise(seed, scale, width, height, octaves,
                                                                      lacunarity, persistence);
                                                  return fractal.Generate();

                                        case NoiseType.Gradient:
                                                  var gradient = new GradientNoise(seed, scale, width, height,
                                                                      amplitude, frequency, offset);
                                                  return gradient.Generate();

                                        case NoiseType.SparseConvolution:
                                                  var sparse = new SparseConvolutionNoise(seed, scale, width, height,
                                                                      kernelSize);
                                                  return sparse.Generate();

                                        case NoiseType.Gabor:
                                                  var gabor = new GaborNoise(seed, scale, width, height, frequency,
                                                                      orientation, aspectRatio, phase, amplitude);
                    ...
                    public static float[] CombineNoise(CombineMethod method, float[][] noiseArrays)
                    {
                              ...
                              var noiseInputs =
                                                  new NativeArray<float>(noiseCount * noiseLength, Allocator.TempJob);
                              var combinedNoise = new NativeArray<float>(noiseLength, Allocator.TempJob);

                              for (int i = 0; i < noiseCount; i++)
                              {
                                        NativeArray<float>.Copy(noiseArrays[i], 0, noiseInputs, i * noiseLength,
                                                            noiseLength);
                              }
